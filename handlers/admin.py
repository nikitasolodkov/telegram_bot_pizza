...
async def make_changes_command(message: types.Message):
    ...
    await bot.send_message(message.from_user.id, 'Чего надобно, хозяин?', reply_markup=admin_kb.button_case_admin)
...
async def cancel_handler(message: types.Message, state: FSMContext):
    if message.from_user.id == ID:
        ...
        if current_state is None:
            return
        await state.finish()
        await message.reply('OK, произошла Отмена')
...
# Ловим последний ответ (цена) и используем полученные данные
...
async def load_price(message: types.Message, state: FSMContext):
    ...
    # sql_add(state)
    # Команда state.finish() сотрёт временный словарь, сохранять надо до неё
    # await state.finish()
